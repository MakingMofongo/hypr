...
# Helper function to copy to both Wayland and X11 clipboards
copy_to_clipboard() {
    local file="$1"
    if [[ -f "$file" ]]; then
        wl-copy < "$file"
...
shotwin() {
	...
	cd ${dir} && grim -g "$w_pos $w_size" "$file"
	copy_to_clipboard "${dir}/${file}"
	notify_view
}
...
shotactive() {
    ...
    copy_to_clipboard "${active_window_path}"
	sleep 1
    notify_view "active"
}
